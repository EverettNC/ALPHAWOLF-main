...
class UserInteraction(db.Model):
    ...
    def __repr__(self):
        return f'<UserInteraction {self.user_id} {self.intent}>'

class CommunicationProfile(db.Model):
    __tablename__ = 'communication_profiles'
    id = Column(Integer, primary_key=True)
    user_id = Column(Integer, ForeignKey('patients.id'), nullable=False, unique=True)
    primary_mode = Column(String(50), default='text')
    secondary_mode = Column(String(50))
    last_updated = Column(DateTime, default=lambda: datetime.now(timezone.utc))

    @classmethod
    def get_latest_profile(cls, user_id):
        return cls.query.filter_by(user_id=user_id).order_by(cls.last_updated.desc()).first()

    def __repr__(self):
        return f'<CommunicationProfile {self.user_id}>'
...
class UserPreference(db.Model):
    ...
    source = Column(String(50)) # system, manual
    timestamp = Column(DateTime, default=lambda: datetime.now(timezone.utc))

    def __repr__(self):
        return f'<UserPreference {self.user_id} {self.type}>'
